...
trait TraceDirectives extends LazyLogging {
  ...
  private def count: Directive1[Long] =
    Directive { innerRouteSupplier => ctx =>
      innerRouteSupplier(
  ...
  def log: Directive0 =
    count flatMap { requestId =>
      mapInnerRoute(addLoggingToRoute(requestId, _))
    }

  private def addLoggingToRoute(requestId: Long, innerRoute: Route): Route = { context =>
    {
      extractClientIP { ip =>
        logger.info(
          s"Http request, id: $requestId, uri: ${context.request.uri}, forwarded ip: $ip"
        )
        mapResponse(httpResponse => {
          logger.info(
            s"Http response, id: $requestId, code: ${httpResponse.status.intValue()}"
          )
          httpResponse
        })(innerRoute)
      }(context)
