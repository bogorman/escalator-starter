...
trait JsonSerializers extends autowire.Serializers[Json, Decoder, Encoder] {
  ...
  override def read[AnyClassToRead](
      json: Json
  )(implicit ev: Decoder[AnyClassToRead]): AnyClassToRead = {
...
object AutowireAkkaHttpRoute {
  ...
  /** @param f
    *   Need to expose this to user in order to not break macro
    * @return
    *   Akka Http route
    */
  def apply(
      uri: PathMatcher[Unit],
      f: AutoWireCirceServer.type => AutoWireCirceServer.Router
  ): Route =
    post {
